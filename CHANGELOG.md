...
## [v1.4.0] - 2026-09-10
### Added — Total Overhaul 1000%
- **💎 Самоцветы и гнёзда (Gems & Sockets)** (`models/item.py`):
  - 6 граней самоцветов: Рубин (огонь), Сапфир (холод), Топаз (молния),
    Изумруд (яд/вампиризм), Аметист (хаос/барьер), Алмаз (крит/поглощение).
  - От 1 до 3 гнёзд на предметах редкости Rare+; статы камня сворачиваются
    прямо в предмет → экипировка и Detailed Stats учитывают их автоматически.
  - `socket_gem` / `unsocket_gem` / `roll_socket_count` / `gem_stats`.
  - Трофеи боссов и элитников падают уже с пробитыми гнёздами и камнями.
- **🎽 Комплекты экипировки (Item Sets)**:
  - 8 легендарных сетов («Клятва Инквизитора», «Пепел Преисподней», «Саван
    Вечной Тьмы», «Ледяная Пытка», «Рождённый Гнилью», «Страж Бури»,
    «Глашатай Могил», «Священный Свет»).
  - Бонусы за 2/4/6 надетых предметов, применяются в `Hero.primary_stats`
    (нулевой вклад, пока нет надетого комплекта — совместимость сохранена).
  - `roll_set_id` на Legendary+ и `apply_item_set_bonuses`.
- **🌍 Живой мир Бездны (8 биомов)** (`engine/biomes.py`):
  - Катакомбы Забытых Королей, Чумные Топи, Заснеженный Пик Мучений,
    Пепельная Кальдера, Осквернённый Собор, Кровавый Зиккурат, Цитадель
    Бездны, Руины Пепельной Луны.
  - Слой ПРЕЗЕНТАЦИИ, отделённый от закреплённого тестами боевого «акта»;
    детерминированная смена локации по волнам, баннер «АКТ N · ЛОКАЦИЯ».
- **⚡ Элементальные статусы (Ailment System)** (`engine/combat_core.py`):
  - Горение (стаки до 5, периодический % урон), Заморозка/Замедление
    (замедление 40%, полная заморозка на 6 стаках с раскалыванием x2),
    Шок (+25% получаемого урона), Кровотечение, Яд (−6% брони/стак и −50%
    лечения), Оглушение. Автономный, покрытый тестами слой.
- **👹 Расширенный бестиарий** (`models/enemy.py`):
  - 12 новых типов монстров (костяные лучники, чумные упыри, болотные ведьмы,
    магматические колоссы, теневые жнецы, глаза искажения и др.) → всего 18+
    уникальных типов; по 5–7 на акт.
  - Спрайты новых мобов прорисованы через уже нарисованные силуэты и
    индивидуальные палитры; данные элитных/боссовых аур и масштаба боссов.
- **🃏 Карточки лута**: гнёзда, самоцветы и комплект показываются в тултипе;
  локация отображается в подсказке главного окна.
- **Тесты**: +24 новых (итого 139/139, все зелёные).

### Changed
- `StatBlock` получил оператор вычитания (для извлечения самоцветов).
- Версия повышена до **1.4.0**.

## [v1.3.0] - 2026-09-10
### Added
- **Лавка Бездны (Module: Shop)** — полноценный торговец:
  - *Продажа лута*: выделенные предметы из рюкзака или тайника превращаются в
    золото по `sell_value × (1 + наценка рун)`. Вкладка «Показывать: Рюкзак/Тайник»,
    кнопка «Распродать хлам» для всех обычных/необычных предметов.
  - *Покупка снаряжения*: полка товаров торговца (до 6 предметов) уровня, близкого
    к среднему уровню отряда; цена = ценность × наценка. Покупка кладётся в рюкзак
    (или тайник, если рюкзак полон). Кнопка «Обновить прилавок».
  - Продажа и покупка полностью **idle** (без активных кликов) и идут через тот же
    атомарный транзакционный движок `UiActions`, что и весь остальной проект.
- **engine/shop.py** — чистая, детерминированная и автономная торговая механика:
  `compute_sell_gold`, `sell_rarity_ids`, `roll_merchant_stock`,
  `merchant_price`, `buy_data`, `junk_ids`.
- **Инвентарь / Сундук**:
  - Фильтр по редкости, счётчик «Показано», суммарная ценность видимого лута.
  - Правый клик по предмету — контекстное меню (забрать / продать / карточка).
  - Кнопка «Продать выбранное» прямо в сундуке.
- **Древо Рун**: поле «Найти руну» — живьём затемняет несовпадающие узлы
  созвездия, подсветка выбранной руны сохраняется.
- **Анимация Куба Синтеза**: вращающаяся пунктирная руна-орбита, пульсирующие
  эфирные точки и вращающаяся гексаграмма вокруг круга трансмутации.
- **115 автотестов** (100 прежних + 15 новых `tests/test_shop.py`), все проходят.

### Changed
- Новая вкладка **ЛАВКА** на док-панели главного окна между «КУБ» и «РУНЫ».
- Цена продажи каждой вещи считается независимо вниз (как при авто-продаже
  переполненного тайника), затем суммируется — поведение стабильно и предсказуемо.
